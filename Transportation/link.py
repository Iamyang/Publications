...
def link(records):
    card_id = records.card_id.value_counts()
    records_2 = records[records.card_id.isin(card_id[card_id >= 2].index)]  # 刷卡记录大于1次才有可能换乘

    drop = []
    for cid, cgroup in records_2.groupby('card_id'):
        cgroup = cgroup.copy()
        cgroup.sort_values(by=['on_time'], inplace=True)
        ...
        while i < n_records - 1:
            ...
            while i < n_records - 1:
                ...
                if dist < dist_thre and interval < time_thre:
                    ...
                    i += 1
                    break
    records.drop(drop, axis='index',inplace=True)
